# -*- coding: utf-8 -*-
"""
Created on Sun Jul  7 00:59:49 2024
...
def bridge(input_tensor, filters, kernel_size, dilation, activation="relu"):
    ...
    # x = bn_activation(x, activation=activation)

    x2 = Conv2D(filters, kernel_size, strides=(1, 1), dilation_rate=dilation, padding='same')(input_tensor)
    x2 = Dropout(.2)(x2)
    x2 = bn_activation(x2, activation)
    
    # x0 = Conv2D(filters, kernel_size, strides=(1, 1), dilation_rate=dilation, padding='same')(x0)
    # x0 = bn_activation(x0, activation)
...
def conv_block_exp_path(input_tensor, filters, kernel_size, activation="relu"):
    """It Is only the convolution part inside each expanding path's block
    """

    x = Conv2D(filters, kernel_size, padding='same')(input_tensor)
    x = bn_activation(x, activation)
...
def feed_fodward_block(input_tensors, conv0, pure_ff, activation="relu"):
    ...
    ama = Activation(activation)(ama)
    return ama


def extracting_block(prev_tensor, conv0, pure_ff, filters, kernel_size=(2, 2), strides=(2, 2), input_tensors=None, activation="relu"):
    ...
    conv = conv_block_exp_path(up, filters, kernel_size, activation)
    
    return add_mul_add_block([conv, pure_ff, convT])
