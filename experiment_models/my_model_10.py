...
def conv_block_exp_path3(input_tensor, filters, kernel_size, activation="relu"):
    """
    Convolution part inside each expanding path's block
    """
...
def extracting_block(prev_tensor, skip_layer, filters, kernel_size_T=(2, 2), 
                     kernel_size=(3, 3), strides=(2, 2), activation="relu", conv_3=False):
    ...
    if conv_3:
        conv = conv_block_exp_path3(up, filters, kernel_size, activation)
    else:
...
def build_model(input_rows=192, input_cols=192, num_of_channels=4, num_of_classes=1, activation="relu"):
    ...
    # expanding part
    extract7 = extracting_block(conv6, conv5, 512, conv_3=True)
