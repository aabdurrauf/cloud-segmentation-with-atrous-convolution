# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 09:16:18 2024

@author: ammar
"""

from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, concatenate, Conv2D, DepthwiseConv2D, MaxPooling2D, Conv2DTranspose, BatchNormalization,Activation, Dropout
...
def contracting_block(input_tensor, filters, kernel_size, dilation, activation="relu"):
    ...
    x1 = DepthwiseConv2D(kernel_size, padding='same')(input_tensor)
    x1 = bn_activation(x1, activation)

    x1 = DepthwiseConv2D(kernel_size, padding='same')(x1)
    x1 = bn_activation(x1, activation=activation)

    x2 = DepthwiseConv2D(kernel_size, dilation_rate=dilation, padding='same')(input_tensor)
    x2 = bn_activation(x2, activation)
    
    x2 = DepthwiseConv2D(kernel_size, dilation_rate=dilation, padding='same')(x2)
    x2 = bn_activation(x2, activation)
    
    x3 = add_mul_add_block([x1, x2])
    
    x4 = Conv2D(filters, kernel_size, padding='same')(x3)
    x4 = bn_activation(x4, activation)
    
    x4 = Conv2D(filters, kernel_size, padding='same')(x4)
    x4 = bn_activation(x4, activation)

    print('shpae x1:', np.shape(x1))
    print('shape x2:', np.shape(x2))
    print('shape x3:', np.shape(x3))
    print('shape x4:', np.shape(x4), '\n')
