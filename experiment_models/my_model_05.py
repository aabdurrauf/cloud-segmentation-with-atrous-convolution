# -*- coding: utf-8 -*-
"""
Created on Mon Jul  8 16:16:47 2024

@author: ammar
"""

from tensorflow.keras.models import Model
from tensorflow.keras.layers import Input, concatenate, Conv2D, MaxPooling2D, Conv2DTranspose, BatchNormalization,Activation, Dropout, Add
...
def contracting_block(input_tensor, filters, kernel_size, dilation, activation="relu"):
    ...
    # x1 = bn_activation(x1, activation) # off at first
    ...
    # x2 = bn_activation(x2, activation) # off at first
    ...
    x2 = Conv2D(filters, kernel_size_b, padding='same')(x2)
    x2 = bn_activation(x2, activation)

    x3 = add_mul_add_block([x1, x2])
    
    return Activation("tanh")(x3)
...
def feed_fodward_block(input_tensors, pure_ff, activation="relu"):
    ...
    if len(input_tensors) > 1:
        for ix in range(len(x_list)-1):
            if ix == 0:
                add = Add()([x_list[0], x_list[1]])
            else:
                add = Add()([add, x_list[ix+1]])
    else:
        add = x_list[0]
...
def build_model(input_rows=192, input_cols=192, num_of_channels=4, num_of_classes=1, activation="relu"):
    ...
    # contracting part
    conv0 = Conv2D(16, (9, 9), activation=activation, padding='same')(inputs)
    
    conv1 = contracting_block(conv0, 32, (7, 7), (2, 2), activation)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    
    conv2 = contracting_block(pool1, 64, (7, 7), (2, 2), activation)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    
    conv3 = contracting_block(pool2, 128, (5, 5), (2, 2), activation)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    
    conv4 = contracting_block(pool3, 256, (5, 5), (2, 2), activation)
    ...
    # expanding part
    extract7 = extracting_block(conv6, conv5, 512, input_tensors=[conv4, conv3, conv2, conv1])
    
    extract8 = extracting_block(extract7, conv4, 256, input_tensors=[conv3, conv2, conv1])
    
    extract9 = extracting_block(extract8, conv3, 128, input_tensors=[conv2, conv1])
    
    extract10 = extracting_block(extract9, conv2, 64, input_tensors=[conv1])
    
    extract11 = extracting_block(extract10, conv1, 32, input_tensors=None)
