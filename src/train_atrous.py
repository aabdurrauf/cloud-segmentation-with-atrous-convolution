...
experiment_name = "AtrousCloud-Net-GS-epoch-50" #+ str(num_epochs)
weights_path = os.path.join(GLOBAL_PATH, 'saved_model_weights', experiment_name + '.h5')
train_resume = True
prev_weight = os.path.join(GLOBAL_PATH, 'saved_model_weights', 'AtrousCloud-Net-GS-epoch-30.h5')
...
def train():
    model = build_model(input_rows=height,
                        input_cols=width,
